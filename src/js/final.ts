...
import { CallbackCollection, COLLECTION, GLOBAL } from "../pipe/tempDB"
...
window.addEventListener("DOMContentLoaded", () => {
    global.ipcRenderer.send(ipcChannelPIPE.MAIN.TEMPDB_COLLECTION, { "path": COLLECTION.PACKAGECOLLECTION, "returnChannel": COLLECTION.PACKAGECOLLECTION })

    global.ipcRenderer.on(COLLECTION.PACKAGECOLLECTION, async (event, collection: CallbackCollection[]) => {
        ...
        try {
            await file.writeFileSync(path.join(defaultPath, "start.xyzerconfig"), stringify({
                "title": consoleTitle,
